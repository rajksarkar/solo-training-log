/**
 * Canonical exercise → muscles mapping shared by `import-th.ts` (so newly
 * imported exercises get tagged) and `backfill-muscles.ts` (one-off rescue
 * for previously-imported exercises with empty muscles arrays).
 *
 * The first entry in each `muscles` array is the primary mover (1.0x credit
 * in /api/volume). Remaining entries are secondaries (0.5x credit). Use the
 * vocabulary the volume route's `normalizeMuscle` already understands:
 *   lats, rhomboids, traps, upper back, lower back   → back
 *   quadriceps                                        → quads
 *   rear delts, front delts, side delts, deltoids     → shoulders
 *   pecs, pectorals, upper chest, lower chest         → chest
 *   hams                                              → hamstrings
 *   abs, obliques, abdominals                         → core
 */

export interface MuscleAssignment {
  /** Exercise name(s), matched case-insensitively. Aliases share one mapping. */
  names: string[];
  /** Primary mover first; remaining entries are secondaries. */
  muscles: string[];
}

export const MUSCLE_ASSIGNMENTS: MuscleAssignment[] = [
  // ─── Pulling / Back ─────────────────────────────────────────────────────
...
// Build lookup index keyed by lowercased exercise name. Whitespace is
// preserved so "slide board  lateral lunge" (which has a double space
// in the source data) still matches.
const INDEX = new Map<string, string[]>();
for (const entry of MUSCLE_ASSIGNMENTS) {
  for (const name of entry.names) {
    INDEX.set(name.toLowerCase(), entry.muscles);
  }
}

/**
 * Look up the muscles array for an exercise by name (case-insensitive).
 * Returns an empty array if the exercise isn't in the mapping — callers
 * should treat that as "leave muscles empty for now".
 */
export function lookupMuscles(name: string): string[] {
